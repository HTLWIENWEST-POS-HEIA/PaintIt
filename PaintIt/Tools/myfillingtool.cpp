...
void MyFillingTool::fill(int x, int y, QColor fillingColor) {
   ...
   QColor replacingColor = QColor(myImage.pixel(x,y));
 // QColor replacingColor = myImage.pixelColor(x, y);

    while(!to_draw.empty()) {
        auto top = to_draw.front();
        to_draw.pop();
        if((top.x >= 0 && top.x < width) && (top.y >= 0 && top.y < height) && (QColor(myImage.pixel(top.x,top.y)) == replacingColor && QColor(myImage.pixel(top.x, top.y)) != fillingColor)) {
     // if((top.x >= 0 && top.x < width) && (top.y >= 0 && top.y < height) && (myImage.pixelColor(top.x, top.y) == replacingColor && myImage.pixelColor(top.x, top.y) != fillingColor)) {
            ...
            myImage.setPixel(QPoint(top.x, top.y), /*QColor*/(fillingColor.rgb()));
